...
def insert_events(events):
    ...
    for i, (parking_id, pairs) in enumerate(events):
        for metric, value in pairs.items():
            row = {
                'id': None,  # Lo puedes omitir si BigQuery usa autoincrement o UUID
...
def get_current_info():
    ...
    query = f"""
    SELECT 
        p.name, p.id AS id_parking, a.sagulpa_id, a.num_spaces
    FROM `{TABLE_PARKINGS}` p
    JOIN `{TABLE_PARKINGS_STATES}` a
      ON p.parkings_states_id = a.id
    WHERE a.is_open = true
    """
    df = client.query(query).to_dataframe()

    return {
        row['name']: [row['id_parking'], row['sagulpa_id'], row['num_spaces']]
